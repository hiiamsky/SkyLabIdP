...
public class SkyLabDevelopLoginUserInfoService(LoginUserInfoServiceSettings loginUserInfoServiceSettings) : AbstractLoginUserInfoService(loginUserInfoServiceSettings)
{
    ...
    {
        return await _unitOfWork.SkyLabDevelopUserDetails.GetByUserIdAsync(userId, cancellationToken) ?? new SkyLabDevelopUserDetail();
    }

    protected override async Task UpdateLastLoginTimeAsync(string userId, CancellationToken cancellationToken)
    {
        await _unitOfWork.SkyLabDevelopUserDetails.UpdateLastLoginTimeAsync(userId, DateTime.Now, cancellationToken);
    }
    ...
    {
        _logger.LogInformation("Creating new user detail:{table} for userId: {UserId}, username: {Username}, fullName: {FullName}, email: {Email}", "SkyLabDevelopUserDetail", userId, username, fullName, email);
        var newUserDetail = new SkyLabDevelopUserDetail
        {
            UserId = userId,
            FullName = fullName,
            OfficialEmail = email,
        ...
        await _unitOfWork.SkyLabDevelopUserDetails.AddAsync(newUserDetail, cancellationToken);
    ...
    protected override async Task UpdateUserDetailByTenantAsync(object userDetail, ApplicationUser user, ExternalUserRegistrationDto userDetails, CancellationToken cancellationToken)
    {
        if (userDetail is SkyLabDevelopUserDetail detail)
        {
            ...
            _logger.LogDebug("更新的詳細資料: 機構={Agency}, 電話={Phone}, 單位={Unit}, 職稱={Title}",
                userDetails.BranchCode, userDetails.OfficialPhone,
                userDetails.SubordinateUnit, userDetails.JobTitle);

            await _unitOfWork.SkyLabDevelopUserDetails.UpdateAsync(detail, cancellationToken);
        }
    }
}
