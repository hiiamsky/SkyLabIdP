...
public interface ISkyLabDocUserDetailRepository
{
    ...
    Task<(IEnumerable<SkyLabDocUserDetailDto> Items, int TotalCount)> GetAccountQueryAsync(AccountQuery request, IEnumerable<string>? userIds = null, CancellationToken cancellationToken = default);
